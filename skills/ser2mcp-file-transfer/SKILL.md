...
description: 使用 ser2mcp 经 UART/COM 串口发送本地文件或固件。用于 uart_send_estimate、uart_send_file、uart_send_cancel，选择 text/base64 与 chunk_size/gap_ms，准备 Linux tty 接收端，处理 EOF/取消/设备断开，并用对端长度和哈希验证完整性。
---
...
4. 准备对端接收，再调用一次：
   ...
   ```text
   uart_send_file {port, path, mode?, chunk_size?, gap_ms?}
   ```
...
- `reason="cancelled"`：由 `uart_send_cancel`、目标端口的 `uart_close` 或客户端取消通知中止。检查对端残留并清理后再决定是否重发。
